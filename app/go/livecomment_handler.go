...
func moderateHandler(c echo.Context) error {
	...
	if err := tx.SelectContext(ctx, &livecomments, "SELECT * FROM livecomments l WHERE l.livestream_id = ?", livestreamID); err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to get livecomments: "+err.Error())
	}

	livecomments = lo.Filter(livecomments, func(c *LivecommentModel, i int) bool {
		for _, ngword := range ngwords {
			if strings.Index(c.Comment, ngword.Word) != -1 {
				return true
			}
		}
		return false
	})
	livecommentIDs := lo.Map(livecomments, func(c *LivecommentModel, i int) int64 {
		return c.ID
	})
	query, args, _ := sqlx.In("DELETE FROM livecomments WHERE id IN (?)", livecommentIDs)
